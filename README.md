...
| ファイル | 役割 |
|---|---|
| `30min-session-strategy.pine` | 30分足切替前後戦略（S1〜S9セッション）。Pine Script v6。各セッションのエントリー/エグジット時刻は設定パネルから変更可能。**1分足での実行が前提**（理由は「セッション時刻の設定について」参照）。 |
...
`30min-session-strategy.pine` の各セッション（S1〜S9）のエントリー/エグジット時刻は、TradingViewの設定パネルからセッションごとに変更できる（例: S1のエントリー時=9、分=28）。変更すると、テーブル表示のラベル（`S1(9:28-...)`等）も自動的に追従する。
